...
//		$('#quranSideBar').hide();
	...
	display: function (success)
	{
		...
		//this.currentAyahView(gq.quran.text());
	...
	currentAyahView: function (quranArray)
	{
		...
		$.each(ayahList, function(i, aboutAyah)
		{
			...
			$.each(quranArray, function(quranBy, text) 
			{
				...
				if (byList[quranBy].type == 'quran' && val !== undefined)
				{
					...
					body += '<p class="ayah '+quranClass+' '+direction+'" dir="'+direction+'" '+fontFamily+' data-verse="'+verseNo+'">'+layout.verseParse(quranBy, val)+'</p>';
			...
			$.each(quranArray, function(quranBy, text)
			{
				...
				{					
					...
					name = by.native_name || by.english_name;
					direction = (gq.quran.direction(quranBy) == 'right') ? 'rtl' : 'ltr';
					body += '<p class="ayah '+direction+'" dir="'+direction+'" data-verse="'+verseNo+'"><a href="'+gq.url.hashless()+'#!/'+quranBy+'/'+val.surah+':'+val.ayah+'" class="quranID">'+name+'</a> '+layout.verseParse(quranBy, val)+'</p>';
				}				
			});
